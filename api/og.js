...
// Vercel serverless function. Crawlers (X, Discord, iMessage, etc.) get
// meta-tag-injected HTML with a personalized card. Real browsers get a
// redirect straight to the normal profile page.
...
// Bots that read OG tags to build link previews
...
function buildCardSvg({ displayName, username, avatarUrl, efedName } = {}) {
  ...
  const avatarHtml = avatarUrl
    ? `<clipPath id="av"><circle cx="960" cy="315" r="185"/></clipPath>
       <image href="${avatarUrl}" x="775" y="130" width="370" height="370"
    ...
    : `<circle cx="960" cy="315" r="185" fill="rgba(255,255,255,0.04)" stroke="#c8a84b" stroke-width="2" stroke-opacity="0.4"/>
       <text x="960" y="370" font-family="Arial Black,Arial" font-size="160"
         fill="rgba(255,255,255,0.55)" text-anchor="middle">${initial}</text>`;
  ...
  const efedBadge = efed
    ? `<rect x="80" y="${efedY - 28}" width="${Math.min(efed.length * 15 + 36, 500)}" height="40"
         rx="6" fill="rgba(200,168,75,0.12)" stroke="#c8a84b" stroke-width="1" stroke-opacity="0.5"/>
       <text x="98" y="${efedY}" font-family="Arial,sans-serif" font-weight="700"
         font-size="21" fill="#c8a84b" letter-spacing="3">${efed}</text>`
    : '';
  ...
  ${avatarHtml}
  <text x="80" y="${nameY}" font-family="Arial Black,Arial" font-weight="900"
    font-size="${fontSize}" fill="#ffffff" letter-spacing="4">${name}</text>
  ${efedBadge}
  <text x="80" y="${handleY}" font-family="Arial,sans-serif" font-size="27"
    fill="rgba(255,255,255,0.3)">${handle}</text>
  <rect x="80" y="${lineY}" width="72" height="3" fill="#c0000e"/>
  <text x="80" y="${siteY}" font-family="Arial,sans-serif" font-size="20"
    fill="rgba(255,255,255,0.18)" letter-spacing="4">${SITE_NAME.toUpperCase()}</text>
</svg>`;
}
...
function buildFallbackSvg() {
  ...
  <text x="80" y="290" font-family="Arial Black,Arial" font-weight="900"
    font-size="110" fill="#ffffff" letter-spacing="10">EFED</text>
  <text x="80" y="368" font-family="Arial,sans-serif" font-weight="400"
    font-size="44" fill="#c8a84b" letter-spacing="7">STREAMING HUB</text>
  <rect x="80" y="398" width="100" height="3" fill="#c0000e"/>
  <text x="80" y="452" font-family="Arial,sans-serif" font-size="26"
    fill="rgba(255,255,255,0.35)">The home of eFed wrestling</text>
</svg>`;
}
...
export default async function handler(req, res) {
  try {
    ...
    // Real browsers: redirect straight to the actual profile page
    if (!isCrawler(ua)) {
      const dest = username
    ...
    // No username — serve the generic platform card
    if (!username) {
      const html = buildHtml({
        title:       SITE_NAME,
        description: 'The home of eFed wrestling. Watch matches, track careers, and connect with the community.',
        imageUrl:    svgToDataUrl(buildFallbackSvg()),
        pageUrl:     SITE_URL,
      });
    ...
    const pageUrl  = `${SITE_URL}/profile.html?u=${encodeURIComponent(username)}`;
    const imageUrl = svgToDataUrl(buildCardSvg({ displayName, username, avatarUrl, efedName }));
